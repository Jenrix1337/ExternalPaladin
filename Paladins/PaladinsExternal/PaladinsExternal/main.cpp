...
auto Render(FLOAT aWidth, FLOAT aHeight)->VOID
{
	...
	if (CFG.b_MenuShow)
	{
		...
		ImGui::Begin("Jenrix Paladins", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
		...
		else if (CFG.in_tab_index == 1)
		{
			...
			if (CFG.b_Aimbot)
			{
				ImGui::Text("Press Alt to Activate");
				ImGui::Checkbox("Draw FOV", &CFG.b_AimFOV);
				if (CFG.b_AimFOV)
				{
					if (ImGui::ColorEdit4("FOV Color", CFG.fl_FovColor))
					{
				...
				if (CFG.b_AimFOV)
				{
					ImGui::SliderFloat("Field Of View", &CFG.fl_AimFov, 5.f, 90.f);
				}
			}
		}
		ImGui::PopFont();
